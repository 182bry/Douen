const visualizationScript = (() => {
    ...
    function renderLineChart(targetId, points, title) {
        ...
        Plotly.react(targetId, [{ x, y, type: 'scatter', mode: 'lines+markers', line: { color: '#1b9aaa' } }], {
            title,
    ...
    function renderBarChart(targetId, counts) {
        ...
        Plotly.react(targetId, [{ x, y, type: 'bar', marker: { color: '#1b9aaa' } }], {
    ...
    async function refreshVisuals() {
        ...
        renderLineChart('chart-60s', data.charts.traffic_60s || [], 'Packets per Flow in the Last 60 Seconds');
        renderBarChart('chart-bar', data.charts.attack_counts || {});
        renderLineChart('chart-24h', data.charts.traffic_24h || [], 'Packets per Flow in the Last 24 Hours');
